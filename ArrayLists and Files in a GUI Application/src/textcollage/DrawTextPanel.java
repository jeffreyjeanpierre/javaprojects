...
public class DrawTextPanel extends JPanel {
	...
	private ArrayList<DrawTextItem> theString = new ArrayList<DrawTextItem>(0); // change to an ArrayList<DrawTextItem>
																				// !
		...
		protected void paintComponent(Graphics g) {
			...
			if (theString.size() > 0) {
				for (int i = 0; i < theString.size(); i++) {
					theString.get(i).draw(g);
				}
				// last = theString.size() - 1;
				// theString.get(last).draw(g);
			} else {
				// do nothing
			}
	 ...
	 * 
	 * @param e
	 *            the mouse event that was generated when the user clicked
	 */
	...
	public JMenuBar getMenuBar() {
		if (menuBar == null) {
			...
			JMenuItem matrixPepper = new JMenuItem("Matrix'd...");
			matrixPepper.addActionListener(menuHandler);
			saveItem.setAccelerator(KeyStroke.getKeyStroke(commandKey + "N"));
			saveItem.addActionListener(menuHandler);
			fileMenu.add(saveItem);
			fileMenu.add(matrixPepper);
	 ...
	 * 
	 * @param command
	 *            the text of the menu command.
	 */
	private void doMenuCommand(String command) {
		if (command.equals("Save...")) { // save all the string info to a file
			File fileFile = fileChooser.getOutputFile(this, "Select File Name", "textdata.txt");
			if (fileFile == null)
				return;
			try {
				PrintWriter pr = new PrintWriter(fileFile);
				String j = String.valueOf(canvas.getBackground().getRed()) +','+ String.valueOf(canvas.getBackground().getGreen()) + ',' + String.valueOf(canvas.getBackground().getBlue()) +'\n';
				pr.write(j);
				for (int i = 0; i < theString.size(); i++) {
					String k = String.valueOf(theString.get(i).getString()) 
							+ ',' + String.valueOf(theString.get(i).getTextColor().getRed()) 
							+ ',' + String.valueOf(theString.get(i).getTextColor().getGreen()) 
							+ ',' + String.valueOf(theString.get(i).getTextColor().getBlue())
							+ ',' + String.valueOf(theString.get(i).getX())
							+ ',' + String.valueOf(theString.get(i).getY())
							+ '\n';
					pr.write(k);
				}
				pr.close();
			} catch (Exception e) {
				JOptionPane.showMessageDialog(this, "Sorry, an error occurred while trying to save:\n" + e);
			}
		} else if (command.equals("Open...")) { // read a previously saved file, and reconstruct the list of strings
			theString.removeAll(theString);
			File fileFile = fileChooser.getInputFile(this, "Select File Name");
			if (fileFile == null)
				return;
			try {
				Scanner scan = new Scanner(fileFile);
				String bg = scan.nextLine();
				while (scan.hasNextLine()) {
					String[] specs = scan.nextLine().split(",");
					String t = String.valueOf(specs[0]);
					int xpos = Integer.parseInt(specs[4]);
					int ypos = Integer.parseInt(specs[5]);
					
					DrawTextItem item = new DrawTextItem(t, xpos, ypos);
					Color colours = new Color(Integer.parseInt(specs[1]), Integer.parseInt(specs[2]), Integer.parseInt(specs[3]));
					item.setTextColor(colours); // Default is null, meaning default color of the canvas (black).
		...
		} else if (command.equals("Matrix'd...")) { // randomize all strings as numbers
			addRandomNumbers();
			canvas.repaint();
	...
	private void addRandomNumbers() {
		...
		int count = (int) (Math.random() * (13 - 3)) + 13;
		for (int i=0; i< count; i++) {
			int xpos = (int) (Math.random() * (ImageObserver.WIDTH*1000 - 20)) + 20;
			int ypos = (int) (Math.random() * (ImageObserver.HEIGHT*1000- 30)) + 30;
			String t = String.valueOf((int) (Math.random() * (2 - 0)) + 0);
			DrawTextItem item = new DrawTextItem(t, xpos, ypos);
			int red = (int) (Math.random() * (255 - 1)) + 1;
			int green = (int) (Math.random() * (255 - 1)) + 1;
			int blue = (int) (Math.random() * (255 - 1)) + 1;
			Color colours = new Color(red, green, blue);
			item.setTextColor(colours); // Default is null, meaning default color of the canvas (black).
