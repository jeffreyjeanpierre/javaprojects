...
public class BuggySearchAndSort {
    ...
    public static boolean contains(int[] array, int val) {
        for (int i = 0; i < array.length-1; i++) { //every time it check the value from 0 to number of data -1 times loop
            //the if statements checks for the each index with the value contains the same index value the it will return the true and the breaks the loop.
           //before that it will continuously checks the value of the all the index array and return true if exist and return false if not exist
           if (array[i] == val){
               return true;
            }
    ...
    public static void bubbleSort(int[] array) {
        for (int i = 0; i < array.length-1; i++) {
            for (int j = 0; j < array.length-i-1; j++) { //loop up to j variable
    ...
    public static void selectionSort(int[] array) {
        for (int top = array.length - 1; top > 0; top--) {
            int positionOfMax = 0;
            for (int i = 1; i <= top; i++) {
                if (array[i] > array[positionOfMax]) //the array index compare with the array index
                    positionOfMax = i;
            }
    ...
    public static void insertionSort(int[] array) {
        for (int top = 1; top < array.length; top++) {
            ...
            int pos = top - 1;
            while (pos >= 0 && array[pos] > temp) { //loop will runs position of the array greater than equal to 0 times array position is greater than temp value
